...
class TestProp(unittest.TestCase):
    ...
    def test_prop_vBinary(self):
        ...
        self.assertEqual(vBinary.from_ical(txt_ical).value, txt)
    ...
    def test_prop_vBoolean(self):
        ...
        # The roundtrip test
        self.assertEqual(
            vBoolean.from_ical(vBoolean(True).to_ical()).value,
            True
        )
        self.assertEqual(vBoolean.from_ical('true').value, True)
    ...
    def test_prop_vCalAddress(self):
        ...
        self.assertEqual(
            vCalAddress.from_ical(txt).value,
            'MAILTO:maxm@mxm.dk'
        )
    ...
    def test_prop_vFloat(self):
        ...
        self.assertEqual(vFloat.from_ical('42').value, 42.0)
    ...
    def test_prop_vDDDLists(self):
        ...
        dt_list = vDDDLists.from_ical(p).value
    ...
    def test_prop_vDDDTypes(self):
        from ..prop import vDDDTypesFactory

        self.assertTrue(
            isinstance(vDDDTypesFactory.from_ical('20010101T123000').value,
                       datetime)
        )

        self.assertEqual(vDDDTypesFactory.from_ical('20010101T123000Z').value,
                         datetime(2001, 1, 1, 12, 30, tzinfo=pytz.utc))

        self.assertTrue(
            isinstance(vDDDTypesFactory.from_ical('20010101').value, date)
        )

        self.assertEqual(vDDDTypesFactory.from_ical('P31D').value,
                         timedelta(31))
        ...
        # Bad input
        self.assertRaises(ValueError, vDDDTypesFactory, 42)
    ...
    def test_prop_vDate(self):
        ...
        self.assertEqual(vDate.from_ical('20010102').value, date(2001, 1, 2))
    ...
    def test_prop_vDatetime(self):
        ...
        self.assertEqual(vDatetime.from_ical('20000101T120000').value,
        ...
        self.assertEqual(vDatetime.from_ical('20010101T000000').value,
                         datetime(2001, 1, 1, 0, 0))

        self.assertRaises(ValueError, vDatetime.from_ical, '20010101T000000A')

        utc = vDatetime.from_ical('20010101T000000Z').value
        self.assertEqual(vDatetime(utc).to_ical(), b'20010101T000000Z')

        # 1 minute before transition to DST
        dat = vDatetime.from_ical('20120311T015959', 'America/Denver').value
        self.assertEqual(dat.strftime('%Y%m%d%H%M%S %z'),
                         '20120311015959 -0700')

        # After transition to DST
        dat = vDatetime.from_ical('20120311T030000', 'America/Denver').value
        self.assertEqual(dat.strftime('%Y%m%d%H%M%S %z'),
                         '20120311030000 -0600')

        dat = vDatetime.from_ical('20101010T000000', 'Europe/Vienna').value
    ...
    def test_prop_vDuration(self):
        ...
        self.assertEqual(
            vDuration.from_ical('PT1H0M22S').value,
            timedelta(0, 3622)
        )

        self.assertRaises(ValueError, vDuration.from_ical, 'kox')

        self.assertEqual(vDuration.from_ical('-P14D').value, timedelta(-14))
    ...
    def test_prop_vPeriod(self):
        ...
        # Roundtrip
        p = vPeriod.from_ical('20000101T000000/20000102T000000').value
        self.assertEqual(
            p,
        ...
        self.assertEqual(vPeriod.from_ical('20000101T000000/P31D').value,
                         (datetime(2000, 1, 1, 0, 0), timedelta(31)))

        # Roundtrip with absolute time
        p = vPeriod.from_ical('20000101T000000Z/20000102T000000Z').value
        self.assertEqual(vPeriod(p).to_ical(),
    ...
    def test_prop_vWeekday(self):
        ...
        self.assertEqual(vWeekday.from_ical('mo').value, 'MO')
        self.assertEqual(vWeekday.from_ical('+3mo').value, '+3MO')
    ...
    def test_prop_vFrequency(self):
        ...
        self.assertEqual(
            vFrequency('daily').from_ical('MONTHLY').value,
            'MONTHLY'
        )
    ...
    def test_prop_vRecur(self):
        ...
        r = vRecur.from_ical('FREQ=DAILY;INTERVAL=2;COUNT=10').value
        self.assertEqual(r,
    ...
    def test_prop_vText(self):
        ...
        # and parsing?
        self.assertEqual(
            vText.from_ical('Text \\; with escaped\\, chars').value,
            u'Text ; with escaped, chars')

        t = vText.from_ical(
            'A string with\\; some\\\\ characters in\\it'
        ).value
        ...
        #
        self.assertEqual(vText.from_ical(b'Ol\xe9').value, u'Ol\ufffd')
    ...
    def test_prop_vTime(self):
        ...
        self.assertEqual(vTime(time(12, 30, 0)).to_ical(), '123000')
        self.assertEqual(vTime.from_ical('123000').value, time(12, 30))
    ...
    def test_prop_vUri(self):
        ...
        self.assertEqual(vUri.from_ical('http://www.example.com/').value,
    ...
    def test_prop_vGeo(self):
        ...
        g = vGeo.from_ical('37.386013;-122.082932').value
    ...
    def test_prop_vUTCOffset(self):
        ...
        self.assertEqual(vUTCOffset.from_ical('0000').value, timedelta(0))
        self.assertEqual(vUTCOffset.from_ical('-0030').value,
                         timedelta(-1, 84600))
        self.assertEqual(vUTCOffset.from_ical('+0200').value,
                         timedelta(0, 7200))
        self.assertEqual(vUTCOffset.from_ical('+023040').value,
                         timedelta(0, 9040))

        self.assertEqual(
            vUTCOffset(vUTCOffset.from_ical('+0230').value).to_ical(),
            '+0230'
        )
    ...
    def test_prop_vInline(self):
        ...
        self.assertEqual(vInline('Some text').to_ical(), 'Some text')
        self.assertEqual(vInline.from_ical('Some text').value, 'Some text')
    ...
    def test_prop_TypesFactory(self):
        ...
        self.assertEqual(
            factory.get(value_type, 'text').from_ical(value).value,
        ...
        self.assertEqual(
            factory.from_ical('cn', b'Rasmussen\\, Max M\xc3\xb8ller').value,
